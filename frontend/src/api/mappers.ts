...
export function mapOfertaFromApi(o: any): Oferta {
  const organizacja = toOrganizacjaMinimal(
    o.organizacja,
    o.organizacja_nazwa ?? "",
  );
  ...
  const normalizeUser = (w: any): Uzytkownik => ({
    ...
    czy_potwierdzone: w.czy_potwierdzone,
    czy_ukonczone: w.czy_ukonczone,
  });
  ...
  if (Array.isArray(o.wolontariusze) && o.wolontariusze.length > 0) {
    ...
    // For backward compatibility, if needed, set the first confirmed one as "main"
    wolontariusz =
      wolontariusze.find((v) => v.czy_potwierdzone) || wolontariusze[0];
  ...
  return {
    ...
    wolontariusze,
    liczba_uczestnikow:
      typeof o.liczba_uczestnikow === "number"
        ? o.liczba_uczestnikow
        : wolontariusz
          ? 1
          : 0,
  };
