...
export default function VolunteerOffersPage() {
  ...
  const topics = useMemo(
    () =>
      Array.from(
        new Set(offers.map((o) => o.tematyka).filter(Boolean)),
      ) as string[],
    [offers],
  );
  const durations = useMemo(
    () =>
      Array.from(
        new Set(offers.map((o) => o.czas_trwania).filter(Boolean)),
      ) as string[],
    [offers],
  );
  ...
  const filtered = offers.filter((o) => {
    if (
      qTopic &&
      qTopic !== "all" &&
      (o.tematyka || "").toLowerCase() !== qTopic.toLowerCase()
    )
      return false;
    if (
      qLocation &&
      !(o.lokalizacja || "").toLowerCase().includes(qLocation.toLowerCase())
    )
      return false;
    if (
      qDuration &&
      qDuration !== "all" &&
      (o.czas_trwania || "").toLowerCase() !== qDuration.toLowerCase()
    )
      return false;
    if (
      qOrganizer &&
      !o.organizacja.nazwa_organizacji
        .toLowerCase()
        .includes(qOrganizer.toLowerCase())
    )
      return false;
    return true;
  });
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <Card className="space-y-2 p-4 gap-0">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              value={qOrganizer}
              onChange={(e) => setQOrganizer(e.target.value)}
            />
          </div>
        </Card>
      </div>
      ...
      <main className="md:col-span-9">
        ...
        {user?.rola === "wolontariusz" && (
          <div
            className={`inline-flex items-center gap-2 mb-4 text-xs px-2 py-1 rounded border ${user?.czy_maloletni ? "bg-yellow-50 border-yellow-300 text-yellow-700" : "bg-emerald-50 border-emerald-300 text-emerald-700"}`}
          >
            Status konta: {user?.czy_maloletni ? "Małoletni" : "Pełnoletni"}
          </div>
        )}
        {filtered.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((oferta) => (
              <Card
                key={oferta.id}
                className="overflow-hidden pt-0 pb-2 gap-4 flex flex-col"
              >
                <div
                ...
                <div className="p-4 pt-0 gap-2 flex flex-col grow">
                  ...
                  <div className="text-xs text-gray-600">
                    {oferta.organizacja.nazwa_organizacji}
                  </div>
                    ...
                    <Button
                      ...
                      disabled={Boolean(
                        oferta.czy_ukonczone ||
                          (user &&
                            ((oferta.wolontariusze || []).some(
                              (u) => u.id === user.id,
                            ) ||
                              (oferta.wolontariusz &&
                                oferta.wolontariusz.id === user.id))),
                      )}
                    >
                      {oferta.czy_ukonczone ? "Zakończona" : "Aplikuj"}
                    </Button>
                    <Link
